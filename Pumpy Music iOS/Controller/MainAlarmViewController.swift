...
class MainAlarmViewController: UITableViewController{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let plistStrAttr: [NSAttributedString.Key : Any] = [NSAttributedString.Key(rawValue: NSAttributedString.Key.font.rawValue) : UIFont.systemFont(ofSize: 20)]
        let plistStr = NSMutableAttributedString(string: alarm.mediaLabel, attributes: plistStrAttr)
        let timeStrAttr: [NSAttributedString.Key : Any] = [NSAttributedString.Key(rawValue: NSAttributedString.Key.font.rawValue) : UIFont.systemFont(ofSize: 15)]
        let daysStr: String = WeekdaysViewController.repeatText(weekdays: alarm.repeatWeekdays)
        let timeStr = NSMutableAttributedString(string: "\(alarm.formattedTime) – \(daysStr)", attributes: timeStrAttr)
        cell!.textLabel?.attributedText = plistStr
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Id.addSegueIdentifier {
            addEditController.navigationItem.title = "Add Scheduled Playlist"
            addEditController.segueInfo = SegueInfo(curCellIndex: alarmModel.count, isEditMode: false, label: "label", mediaLabel: "Enter Playlist Here", mediaID: "", repeatWeekdays: [1, 2, 3, 4, 5, 6, 7], enabled: true, snoozeEnabled: false)
        }
        else if segue.identifier == Id.editSegueIdentifier {
            addEditController.navigationItem.title = "Edit Scheduled Playlist"
    ...
    public func changeSwitchButtonState(index: Int) {
        ...
        if alarmModel.alarms[index].repeatWeekdays.isEmpty {
            alarmModel.alarms[index].enabled = true
        }
        let cells = tableView.visibleCells
        for cell in cells {
            if cell.tag == index {
